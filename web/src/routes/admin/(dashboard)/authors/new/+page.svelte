<script lang="ts">
	...
	async function saveAuthor() {
		...
		try {
			...
			if (response.ok) {
				...
				showLogger = true;
				loggerMsg = `Email a urlSlug musí být unikátní a všechna pole musí být vyplňena - ${response.message}`;
				type = 'error';
			}
		} catch (error) {
			showLogger = true;
	...
	function makeUrlSlug() {
		...
		urlSlug = (randomValue + firstName + lastName).toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-').normalize("NFD").replace(/[\u0300-\u036f]/g, "");
	}
</script>
